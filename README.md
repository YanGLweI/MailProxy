...
| ✅ **统一接入点** | 🔑 **多账号路由** |
|------------------|------------------|
| 所有业务程序对接同一台 SMTP 服务器，无需感知后端邮箱服务商差异 | 根据发件人自动选择最优后端账号，提升送达率 |

| 🚀 **零代码改造** | 🛡️ **安全合规** | 💼 **企业邮箱兼容** |
|------------------|------------------|------------------|
| 只需修改 SMTP 地址配置，现有发邮件逻辑完全不用变 | IP 白名单 + 可选认证，防止被滥用为开放中继 | 自动处理「信封发件人必须等于认证账号」限制（阿里/腾讯/网易等） |
...
![MailProxy Architecture](./docs/architecture.svg)
...
- 🛡️ 对外提供 **SMTP over SSL（465 端口）** 标准 SMTP 服务，兼容 `EHLO/HELO、AUTH、MAIL FROM、RCPT TO、DATA、QUIT`
...
- ✉️ 可选信封发件人改写（后端 `rewrite_from`）：对要求「信封发件人==认证账号」的后端（如企业邮箱），转发前把 MAIL FROM 改写为后端账号，报文头 From 不变
- 🔄 后端转发支持 SSL/TLS（465）与 STARTTLS（587），后端错误原样透传给业务客户端，不静默丢邮件
- ⚙️ 配置文件（YAML）管理，**SIGHUP 热加载**，无需重启（监听地址/证书变更除外）
- ✅ 启动时对每组后端做连通性 + 认证检测
- 🛑 IP 白名单访问控制，防止被当作开放中继
- 🎚️ 并发连接数上限、TLS 握手超时、收发超时，避免僵死连接
...
**⚠️ 注意**: `server.listen`、`server.starttls_listen` 与 TLS 证书变更需要重启。热加载校验失败时沿用旧配置并记录错误日志。
